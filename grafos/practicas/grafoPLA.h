...
#include <string>
#include <ostream>
#include "../estructuras/listaenla.h"
...
std::ostream& operator <<(std::ostream& os, const GrafoP<T>& G)
{
   ...
   for (vertice i = 0; i < n; i++)
      if (G.adyacentes(i).primera() != G.adyacentes(i).fin()) {
         os << i << ':';
         for (posicion p = G.adyacentes(i).primera(); p = G.adyacentes(i).siguiente(p))
            os << ' ' << G.adyacentes(i).elemento(p).v << ' ' << G.adyacentes(i).elemento(p).c;
         os << std::endl;
      }
   return os;
}

#endif   // GRAFO_PON_LIS_ADY_
